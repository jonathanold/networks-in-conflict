args temp_active temp_active_d
...
/*----------------------------------------------------*/
   /* [>   CHANGE.  Here, only use active periods to determine fighting efforts   <] */ 
/*----------------------------------------------------*/

merge m:1 group year using `temp_active', keepusing(active)
drop _merge 
merge m:1 group_d year using `temp_active_d', keepusing(active_d)
drop _merge
** option **  
* gen Fighting_Enemy_d = (Totfighting_d - Nfighting) * (enemy==1)
 gen Fighting_Enemy_d = (Totfighting_d) * (enemy==1) * (active==1) * (active_d==1)

** option **   
* gen Fighting_Allied_d = (Totfighting_d - Nalliance) * (allied==1)
 gen Fighting_Allied_d = (Totfighting_d) * (allied==1) * (active==1) * (active_d==1)

** option **  
gen Fighting_Neutral_d = (Totfighting_d) * (neutral==1) * (active==1) * (active_d==1)

* Check here what happens with negative values of Fighting_allied: 
* I guess this comes from removing alliances with three partners + in some events tehre is no identified enemy in the raw data  
...
rename Fighting_Enemy_d TotFight_Enemy_TIME
rename Fighting_Allied_d TotFight_Allied_TIME
rename Fighting_Neutral_d TotFight_Neutral_TIME
...
keep group year TotFight_Enemy_TIME TotFight_Allied_TIME TotFight_Neutral_TIME
save KRTZ_monadic_base_mt_TIME.dta,replace
