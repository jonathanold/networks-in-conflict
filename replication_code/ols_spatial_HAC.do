...
/*-----------------------------------------------------------------------------
 ...
 discontinously falls from 1 to zero at length locCutoff in all directions (it is isotropic). This is the kernel recommented by Conley (2008). If the option "bartlett" is selected, a conical kernel that decays linearly with distance in all directions is used instead.
...
for (ti = 1; ti <= Ntime; ti++){
	...
	for (i = 1; i <=n1; i++){
		...
		inner = sqrt( ///
				sin((lat1[i,1]:-lat1)*d2r/2):^2 + ///
				cos(lat1*d2r):*cos(lat1[i,1]*d2r) * ///
				sin((lon1[i,1]:-lon1) * d2r / 2):^2)


		distance_j = 2:*asin(mm_cond(inner:>=1, 1, inner)):*6371
		...
		window_i = distance_j :<= dist_cutoff
		...
		if ("`bartlett'"=="bartlett"){
			...
			weight_i = 1:- distance_j:/dist_cutoff
...
for (pi = 1; pi <= Npanel; pi++){
	...
	for (t = 1; t <=n1; t++){
        ...
        // this is the weight for Newey-West with a Bartlett kernel
